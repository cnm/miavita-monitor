...
function formatTooltipsBar(nodeID) {
	var fails = retrieveData(nodeID, Parameter.fails);
	var retries = retrieveData(nodeID, Parameter.retries);
	var sequence = retrieveData(nodeID, Parameter.sequence);
	...
	for (var i = 0; j < size; j++) {
		...
		if (j != size) result[j] = new String("'Sequence: " + sequence[i] + ", Fails: " + fails[i] + "'");
			i++;
	}
...
function formatTooltipsLine(nodeID) {
	var air_time = retrieveData(nodeID, Parameter.air_time);
	var sequence = retrieveData(nodeID, Parameter.sequence);
...
function drawNetwork(node) {
	...
	bar.Set('chart.labels', relabel(retrieveData(node, Parameter.timestamp)));
	...
	var line = new RGraph.Line(graph, retrieveData(node, Parameter.air_time));
...
function stuff(node){
	...
	result.push(retrieveData(node, Parameter.sample1));
	result.push(retrieveData(node, Parameter.sample2));
	result.push(retrieveData(node, Parameter.sample3));
...
function drawsys(node) {
	...
	var options = {
		...
		title: {
			text: 'Sysmology Chart for Node ' + node,
			x: -20 //center
		},
		xAxis: {
			title: {
	...
	if (!$('#sysRT').is(':checked')){
		...
		options.series = [{
				name: 'X axis',
				data: retrieveData(node, Parameter.sample1)
				}, {
				name: 'Y axis',
				data: retrieveData(node, Parameter.sample2)
				}, {
				name: 'Z axis',
				data: retrieveData(node, Parameter.sample3)
		}];
	}
	else {
		options.chart = {
